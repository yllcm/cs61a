...
def balanced(m):
    ...
    right_arm = right(m)
    def get_torque(m):
        """Get the torque applied by a certain arm"""
        return total_mass(end(m)) * length(m)
    left_torque = get_torque(left_arm)
    right_torque = get_torque(right_arm)
...
def max_path_sum(t):
    ...
    def calc_sum(t, start):
        ...
        result = 0
        for branch in branches:
            result = max(result, calc_sum(branch, start))
        return result
    return calc_sum(t, 0)
