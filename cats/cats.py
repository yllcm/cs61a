...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    available = [s for s in paragraphs if select(s)]
    if len(available) <= k:
        return ""
    else:
        return available[k]
...
def about(subject):
    ...
    def select(paragraph):
        ...
        def is_in_subject(s):
            return any(s == x for x in subject)
        return any(is_in_subject(x) for x in words)
...
def accuracy(typed, source):
    ...
    count = len([i for i in range(min_length) if typed_words[i] == source_words[i]])
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        "*** YOUR CODE HERE ***"
        state = (typed, source)
        # print('typed', typed, 'source', source, state in cache)
        if state in cache and (cache[state][0] >= cache[state][1] or cache[state][0] >= limit):
            # print('no calling!')
            return min(limit + 1, cache[state][1])
        else:
            result = diff_function(typed, source, limit)
            cache[state] = [limit, result]
            return result
        # END PROBLEM EC
...
def furry_fixes(typed, source, limit):
    ...
    def count(start, start_num):
        if start >= min_length:
            return start_num
        if start_num > limit:
            return start_num
        return count(start + 1, start_num + (typed[start] != source[start]))
    return count(0, length_diff)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    min_length = min(len(typed), len(source))
    mistakes = [i for i in range(min_length) if typed[i] != source[i]]
    if not mistakes:
        correct_num = min_length
    else:
        correct_num = mistakes[0]
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_player = [[words[j] for j in word_indices if i == min(player_indices, key = lambda x : times[x][j])] for i in player_indices]
