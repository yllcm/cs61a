...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive and step <= self.upper_bound:
            if step >= self.lower_bound and len(current_place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            Bee.reduce_health(bee, reflective_damage)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def make_action(cooldown):

            def action(gamestate):
                if action.cooldown == 0 or gamestate.time % 2 == 0:
                    Bee.action(target, gamestate)
                action.cooldown = max(0, action.cooldown - 1)
            
            action.cooldown = self.slow_cooldown
            return action
        
        target.action = make_action(self.slow_cooldown)
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        return max(0, self.damage - (self.insects_shot * 1 / 16) - (distance * 1 / 4))
        # END Problem EC 4
