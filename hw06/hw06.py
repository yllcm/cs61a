...
def store_digits(n):
    ...
    def digits(n):
        if n < 10:
            single_digit = Link(n)
            return single_digit, single_digit
        else:
            digits_link, last_digit = digits(n // 10)
            last_digit.rest = Link(n % 10)
            return digits_link, last_digit.rest
    
    digits_link, last_digit = digits(n)
    return digits_link
...
def two_list(vals, counts):
    ...
    "*** YOUR CODE HERE ***"
    result = Link.empty
    vals_and_counts = reversed(list(zip(vals, counts)))
